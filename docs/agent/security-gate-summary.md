...
The corrected candidate scan (86 files at that time; the manifest is now 111, and the 2026-09-24 counts are in `SUBMISSION_GAP_CHECK.md`) reports AI scanner 0 CRITICAL / 4 HIGH / 63 MEDIUM (export-gate PASS), release-boundary 0 findings, and LocalGuard 2 CRITICAL / 12 HIGH / 40 MEDIUM. The two additional LocalGuard HIGH hits come from the restored language files above. Raw findings are not suppressed; owner exception approval remains pending.
